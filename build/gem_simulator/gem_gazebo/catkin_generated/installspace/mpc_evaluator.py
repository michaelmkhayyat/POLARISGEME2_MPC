...
class MpcEvaluator(object):
    def __init__(self):
        ...
        self.left_bounadary_points = np.genfromtxt(
            os.path.join(mpc_gazebo_dir, "./data/left_boundary_points.csv"), delimiter=","
        )
        
        self.right_boundary_points = np.genfromtxt(
            os.path.join(mpc_gazebo_dir, "./data/right_boundary_points.csv"), delimiter=","
        )
        
              
        self.obstacles = np.genfromtxt(
            os.path.join(mpc_gazebo_dir, "./data/obstacles.csv"), delimiter=",", skip_header=1
        )
        ...
        self.obstacles_pub = rospy.Publisher(
            "/mpc/obstacles_info", Detection3DArray, queue_size=1
        )
        self.obstacles_rviz_pub = rospy.Publisher(
    ...
    def publish_path(self):
        ...
        self.path_rviz_pub.publish(msg)
            
        msg = Path()
        msg.header.stamp = rospy.Time.now()
        msg.header.frame_id = WORLD_FRAME_ID

        for pt in self.left_bounadary_points:
            pose = PoseStamped()
            pose.pose.position.x = pt[0]
            pose.pose.position.y = pt[1]
            pose.pose.orientation.x = 0.0
            ...
            pose.pose.orientation.w = 1.0
            msg.poses.append(pose)
        self.path_boundary_l_rviz_pub.publish(msg)
        
        msg = Path()
        msg.header.stamp = rospy.Time.now()
        msg.header.frame_id = WORLD_FRAME_ID

        for pt in self.right_boundary_points:
            pose = PoseStamped()
            pose.pose.position.x = pt[0]
            pose.pose.position.y = pt[1]
            pose.pose.orientation.x = 0.0
            ...
            pose.pose.orientation.w = 1.0
            msg.poses.append(pose)
        self.path_boundary_r_rviz_pub.publish(msg)
    ...
    def publish_obstacles(self):
        ...
        for idx, o in enumerate(self.obstacles):
            ...
            msg_rviz.markers.append(marker)
            ...
            msg_rviz.markers.append(marker)
        self.obstacles_rviz_pub.publish(msg_rviz)
